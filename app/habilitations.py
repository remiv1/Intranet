...
def validate_habilitation(
        required_habilitation: str | List[str]
    ) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
    """
    Décorateur pour valider si l'utilisateur connecté possède une habilitation spécifique.

    Args:
        required_habilitation (str | List[str]): Habilitation requise (ex: '3').

    Returns:
        Callable[[Callable[..., Any]], Callable[..., Any]]:
            La fonction décorée ou une réponse d'erreur si l'habilitation est manquante.
    """
    def decorator(function: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            for rh in required_habilitation_list:
                logger.debug("Vérification de l'habilitation requise : %s", rh)
                for habilitation in habilitations:
                    logger.debug("Comparaison avec l'habilitation utilisateur : %s", habilitation)
                    if habilitation == rh:
                        logger.debug("Habilitation valide trouvée : %s", habilitation)
                        is_habilitated = True
                        break
            if not is_habilitated:
                logger.warning(
                    "Habilitation manquante. Requise : %s, Possédée : %s",
