...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        try:
            habilitation = session.get('habilitation', '')
            if '1' not in str(habilitation):
                raise NotAdminError
...
def administrator_required(f):
    ...
    def decorated_function(*args, **kwargs):
        try:
            habilitation = session.get('habilitation', '')
            if '2' not in str(habilitation):
                raise NotAdministratorError
        except NotAdministratorError:
            return redirect(url_for('logout'))
        return f(*args, **kwargs)
    return decorated_function

def admin_or_administrator_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        try:
            habilitation = session.get('habilitation', '')
            ...
            return redirect(url_for('logout', message=str(e)))
        return f(*args, **kwargs)
    return decorated_function
...
def logout():
    session.clear()
    message = request.args.get('message', 'Vous avez été déconnecté avec succès.')
    return redirect(url_for('login', message=message))
...
@admin_or_administrator_required
...
def add_contrats_event(numContrat):
    if request.method == 'POST':
        ...
        raise BadRequestError('Requête HTTP incorrecte, contactez l\'équipe technique.')
...
class NotAdminError(Exception):
    '''Exception raised for errors in the admin check.'''
    status_code = 411
    message = 'Accès réservés aux admin du site intranet (directeur et équipes tech)'
...
@app.errorhandler(NotAdminError)
def handle_not_admin_error(error):
    return logout(), error.status_code

class NotAdministratorError(Exception):
    """Exception raised for errors in the administrator check."""
    status_code = 412
    message = 'Accès réservés aux services administratifs de l\'établissement'
    def __init__(self, message=None, status_code=None):
        if message:
            self.message = message
        if status_code:
...
@app.errorhandler(NotAdministratorError)
def handle_not_administrator_error(error):
    return logout(), error.status_code

class BadRequestError(Exception):
    '''
    Exception levée lors d'une erreur de type de requête HTTP
    '''
    status_code = 410
    message = 'Requête HTTP incorrecte, contactez l\'équipe technique remiv1@gmail.com'
    def __init__(self, message=None, status_code=None):
        if message:
            self.message = message
        if status_code:
...
def handle_bad_request_error(error):
    return logout(), error.status_code
>>>>>>> e4b3d9d (feat: Ajout de vérifications d'habilitation pour les routes administratives et admin et gestion des erreurs)
